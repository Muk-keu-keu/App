...
```
GET v1/posts/{postId}   글 상세. items 가 이미 장바구니 모양
  ↓
프론트가 그 stores 덩어리를 localStorage 장바구니에 push
  ↓
장바구니 화면 (기존 화면 그대로)
  ↓
...
| `selectedSpice` 3단계 | `NONE` \| `MEDIUM` \| `HOT`. Figma 옵션 변경 화면은 "매운맛 5단계" 라 `selectedOptions` 로 들어가는지 확인 필요 |
