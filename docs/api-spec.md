...
| **accessToken 수명이 140일** | `exp - iat` 가 약 140일이고 `refreshToken` 이 `accessToken` 과 같은 문자열이다. 두 토큰을 나눈 의미가 없고, 하나가 새면 넉 달 이상 쓸 수 있다. accessToken 을 짧게(예: 1h) 두고 refreshToken 을 별도 토큰 + 서버측 폐기로 나눠야 한다 |
